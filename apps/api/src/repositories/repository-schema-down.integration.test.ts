...
describe('latest migration rollback', () => {
  it('removes only the repository issues table', async () => {
    const result = await pool.query<{
      ...
      `
        SELECT
          ...
          to_regclass('public.repository_issues')::text
            AS repository_issues
      `,
    );
